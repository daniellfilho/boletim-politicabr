...
- "resumo" deve ter 1 a 2 frases (até 220 caracteres), em texto simples.
- NUNCA use tags de citação, XML, HTML ou qualquer marcação (como
  <cite>, <source>, colchetes de referência etc.) dentro dos textos.
  Escreva sempre em texto puro, sem nenhuma marcação.
- Seja direto e conciso. Não escreva rascunhos, explicações extras ou
  comentários antes ou depois do JSON — vá direto ao JSON final.
...
def gerar_feed_do_dia() -> dict:
    ...
    # Protecao extra: remove qualquer tag residual (ex: <cite ...>...</cite>)
    # que o modelo eventualmente adicione por engano, mantendo so o texto de dentro.
